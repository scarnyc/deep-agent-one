...
// Store original MockWebSocket class
let OriginalMockWebSocketClass: typeof MockWebSocket;
...
// Store the original MockWebSocket class
OriginalMockWebSocketClass = MockWebSocket;
...
describe('useWebSocket Hook', () => {
  beforeEach(() => {
    ...
    // Reset to original MockWebSocket class (in case tests modified it)
    global.WebSocket = OriginalMockWebSocketClass as any;
  });
  ...
  describe('Issue 46: Max Reconnection Attempts Circuit Breaker', () => {
    it.skip('should stop reconnecting after max attempts reached', async () => {
      ...
      // Simulate multiple disconnections to trigger max attempts
      for (let attempt = 1; attempt <= maxReconnectAttempts + 1; attempt++) {
        // Close current connection abnormally
        act(() => {
          const ws = getLatestMockWS();
          if (ws && ws.readyState === MockWebSocket.OPEN) {
            ws.close(1006, 'Network failure');
          }
        });
      ...
      expect(onError).toHaveBeenCalled();
      const hasMaxAttemptsError = onError.mock.calls.some(([err]) =>
