...
cleanup_directory "logs/backend" "backend"

# Cleanup frontend logs
cleanup_directory "logs/frontend" "frontend"

# Show final summary
echo -e "${BLUE}Final Summary:${NC}"
backend_count=$(find logs/backend -name "*.log" -type f 2>/dev/null | wc -l | tr -d ' ')
frontend_count=$(find logs/frontend -name "*.log" -type f 2>/dev/null | wc -l | tr -d ' ')
